...
% File = 'piano'; % Name of file to load
File = 'speech0_female'; % Name of file to load
% File = 'stim7_applause'; % Name of file to load
% File = 'speech_74'; % Name of file to load
...
gapPos = [2000,4000,6000,8000,10000,12000,14000,16000,18000,20000,22000,24000,26000,28000,30000]; % for speech
% gapPos = [700,4700,7500,8500,12500]; % for applause
gapPos(gapPos>=y_max_length*fs_-gapLim(end))=[];
% gapSize = [0.001*fs_, 0.003*fs_, 0.006*fs_, 0.01*fs_, 0.015*fs_];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Also tune parameters with another kernel
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%if trainFilter==1
  % Learn properties of the filters (centre frequency and width)
  opts = struct;
  ...
  opts.bet = 750;  % increase if filters bunch together
  opts.reassign = 0;  % set to 1 if filters really bunch together
  opts.bandwidth_lim = 3;  % limits the bandwidths to n times the initial values
  
  opts.theta_init = [Var1Fit;Lam1Fit;omFit];
  [Var3Fit,Lam3Fit,om3Fit,InfoFit3] = fit_probSTFT_SD(yTrain,D,kernel3,opts); % trains filters to match the spectrum
...
% [A1,Q1,H1,Pinf1,K1,tau1] = get_disc_model(Lam3Fit,Var3Fit,om3Fit,D,kernel3,se_approx_order);
