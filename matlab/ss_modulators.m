function [F,L,Qc,H,Pinf,dF,dQc,dPinf] = ss_modulators(w,kernel1,kernel2)
...
%% periodic subband
  ...
  dF_cos_kron = zeros(tau1*tau2*D+D*tau3,tau1*tau2*D+D*tau3,D); % in NMF model change D*tau3 to N*tau3
  ...
  for d=1:D
    dF_sm_1(:,:,d) = blkdiag(kron(dF1(:,:,d),eye(tau2)),zeros(D*tau3)); % in NMF model change D*tau3 to N*tau3
    dF_sm_2(:,:,d) = blkdiag(kron(dF1(:,:,D+d),eye(tau2)),zeros(D*tau3)); % in NMF model change D*tau3 to N*tau3
    dQc_sm_1(:,:,d) = blkdiag(kron(dQc1(:,:,d),eye(tau2)),zeros(D)); % in NMF model change D to N
    dQc_sm_2(:,:,d) = blkdiag(kron(dQc1(:,:,D+d),eye(tau2)),zeros(D)); % in NMF model change D to N
    dPinf_sm_1(:,:,d) = blkdiag(kron(dPinf1(:,:,d),eye(tau2)),zeros(D*tau3)); % in NMF model change D*tau3 to N*tau3
    dPinf_sm_2(:,:,d) = blkdiag(kron(dPinf1(:,:,D+d),eye(tau2)),zeros(D*tau3)); % in NMF model change D*tau3 to N*tau3
  end
...
%% slow varying modulator
  ...
  F2=[];L2=[];Qc2=[];H2=[];Pinf2=[];
  dF2_1 = zeros(tau3*D,tau3*D,2*D);
  dF2_2 = zeros(tau3*D,tau3*D,2*D);
  dQc2_1 = zeros(D,D,2*D);
  dQc2_2 = zeros(D,D,2*D);
  dPinf2_1 = zeros(tau3*D,tau3*D,2*D);
  dPinf2_2 = zeros(tau3*D,tau3*D,2*D);
  for d=1:D
    ...
    dF2_2(1+(d-1)*tau3:d*tau3,1+(d-1)*tau3:d*tau3,D+d) = dF2d(:,:,2); % d_len
    dQc2_1(d,d,d) = dQc2d(:,:,1); % d_sig
    dQc2_2(d,d,D+d) = dQc2d(:,:,2); % d_len
    dPinf2_1(1+(d-1)*tau3:d*tau3,1+(d-1)*tau3:d*tau3,d) = dPinf2d(:,:,1); % d_sig
    dPinf2_2(1+(d-1)*tau3:d*tau3,1+(d-1)*tau3:d*tau3,D+d) = dPinf2d(:,:,2); % d_len
  end
  dF2 = dF2_1 + dF2_2;
  dQc2 = dQc2_1 + dQc2_2;
