function [varargout] = ihgp_ep_mods_nmf_mixture(w,x,y,ss,mom,xt,kernel1,kernel2,J,ep_fraction,ep_damping,ep_itts)
        ...
        for k=1:numel(yall)
          if mod(k,16000) == 0, fprintf('filtering step %i / %i \n',k,numel(yall)); end
            ...
            if itt == 1 || k == numel(yall)
                ...
                [lZ_k,dlZ,d2lZ] = mom(lik_param,fmu,HPH,Wnmf,yall,k);
                lZ = lZ + lZ_k;
    ...
    %         PS(:,:,k) = P;
            ...
            if itt < ep_itts
                if ~isnan(yall(k))
                  ...
                  [lZ_k,dlZ_,d2lZ_] = mom(lik_param,m_cav,v_cav,Wnmf,yall,k);
                  ...
                  % Moment matching
                  ttau(update_idx, k) = (1-ep_damping)*ttau(update_idx, k) + ep_damping/ep_fraction*(-d2lZ_(update_idx)'./(1+d2lZ_(update_idx)'.*v_cav(update_idx)));
                  tnu(update_idx,k) = (1-ep_damping)*tnu(update_idx, k) + ep_damping/ep_fraction*((dlZ_(update_idx)'-m_cav(update_idx).*d2lZ_(update_idx)')./(1+d2lZ_(update_idx)'.*v_cav(update_idx)));
