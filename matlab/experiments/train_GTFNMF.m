function train_GTFNMF(soundPath,File,opts)

rng(12345,'twister')

fs_ = opts.fs;
train_duration = opts.train_duration;
D = opts.D;
N = opts.N;
kernel1 = opts.kernel1;
kernel2 = opts.kernel2;
link = opts.link;
w_lik_train = opts.w_lik_train;
w_lik_test = opts.w_lik_test;
inf_type = opts.inf_type;
p_cubature = opts.p_cubature;
max_iters = opts.max_iters;
likfunc = opts.likfunc;
ep_fraction = opts.ep_fraction; % Power EP fraction (0.8-1 seems best)
  
  %%% training params %%%
ep_damping_train = opts.ep_damping_train; % EP damping during training
ep_itts_train = opts.ep_itts_train; % EP iterations during training
  
  %%% testing params %%%
ep_damping_test = opts.ep_damping_test; % EP damping during testing (0.1 seems best)
ep_itts_test = opts.ep_itts_test; % EP iterations during testing

l_iter = opts.l_iter; % local EKF (inner loop) iterations
suffix = opts.suffix;
...
%% Load signal and pre-process
  ...
  T = length(y_norm);
  t = linspace(1,T,T)';
  yTrain = y_norm(1:min(train_duration,T));
...
%% Now set up the full model  
  ...
  tune_hypers = [1,     0,     0,     0,     1,     1,     1     ];
  
  
  % pick model parameters
  var_fast = mean(range_var_fast).*ones(D,1);
  len_fast = min(max(1.2*lenx,range_len_fast(1)+1),range_len_fast(end)-1);
...
%% Saving
  ...
  results.w_lik_init = w_lik_train;
  results.w_lik_opt = lik_param;
  results.w_lik_test = w_lik_test;
  ...
  results.p_cubature = p_cubature;
  results.likfunc = likfunc;
  results.l_iter = l_iter;
  results.suffix = suffix;
  matName = strcat('trained_',File,'_',inf_type,suffix,'.mat');
