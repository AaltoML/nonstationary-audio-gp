...
%   minVar(:) = 0.001;
  ...
  if exist(strcat('get_Obj_pSTFT_',kernel),'file')
    Obj_func = strcat('get_Obj_pSTFT_',kernel);
  else
    Obj_func = strcat('get_Obj_pSTFT_','all');
  end
...
% 				  lamLim,bet(c2f)*numFreq(c2f)/numFreq(c2f(1)));
  [theta,ObjCur,inCur] = minimize(theta,Obj_func,numIts, ...
				  vary, specTar,minVar,omLim, ...
				  lamLim,bet(c2f)*numFreq(c2f)/numFreq(c2f(1)), kernel);
...
% 						   omLim,lamLim,0); 
    [likeHO(c2f,1)] = Obj_func_handle(theta,0,specHO,minVar, ...
						   omLim,lamLim,0, kernel);                    
  end
...
% 							 omLim,lamLim,0);
  [likeUnReg(c2f,1)] = Obj_func_handle(theta,0,specUR,minVar, ...
							 omLim,lamLim,0,kernel);
...
if isfield(varargin{1},'reassign')&c2f<numLevels-1
  ...
  if varargin{1}.reassign==1
	...
	disp('moving pruned process')
      ...
      if exist(strcat('get_pSTFT_spec_cts_',kernel),'file')
        specModHandle = str2func(strcat('get_pSTFT_spec_cts_',kernel));
      else
        specModHandle = str2func(strcat('get_pSTFT_spec_cts_','all'));
      end
%       specModHandle = str2func(strcat('get_pSTFT_spec_cts_',kernel));
